...
# test if a nummber is an integer
is_integer <- function(value) {
    if(is.numeric(value) && value %% 1 == 0) return(TRUE)
    else return(FALSE)
}


# test if a number is positive
is_positive <- function(value, convert = FALSE) {
    if(!convert) return(is.numeric(value) && value > 0)
    convertable <- TRUE
    tryCatch(
        {value <- as.numeric(value)},
        warning = function(w) {convertable <<- FALSE},
        error = function(e) {convertable <<- FALSE}
    )
...
is_positive_integer <- function(value, convert = FALSE) {
    if(!convert) return(is_integer(value) && value > 0)
    convertable <- TRUE
    tryCatch(
        {value <- as.numeric(value)},
        warning = function(w) {convertable <<- FALSE},
...
# test if a numberic vec falls into a specified range
between <- function(x, left, right, left_inclusive = T, right_inclusive = T,
                    convert = FALSE) {
    if(isTRUE(convert)) {
        convertable <- TRUE
        tryCatch(
    ...
    if(left_inclusive) result <- result & (x >= left)
    else result <- result & (x > left)
    if(right_inclusive) result <- result & (x <= right)
    else result <- result & (x < right)
    return(result)
}

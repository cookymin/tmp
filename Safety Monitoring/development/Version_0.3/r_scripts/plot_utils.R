...
gg_wrapper <- function(..., facet_r = NULL, facet_c = NULL,
    ...
    if(!x_log) {
        if(any(!is_blank(x_axis_breaks), !is.null(x_axis_labels))) {
            ...
            base_plot <- base_plot +
                x_axis_draw(name = x_lab, breaks = x_axis_breaks,
                            labels = x_axis_labels, expand = x_expand)
        }
    } else {
        force(x_axis_labels)
    ...
    if(!y_log) {
        if(any(!is_blank(y_axis_breaks), !is.null(y_axis_labels))) {
            ...
            base_plot <- base_plot +
                y_axis_draw(name = y_lab, breaks = y_axis_breaks,
                            labels = y_axis_labels, expand = y_expand)
        }
    } else {
        force(y_axis_labels)

...
gg_dotplot <- function(data, x_var, y_var,
    ...
    if(!is_blank(summarize_method)) {
        if(!is.function(summarize_method)) {
            if(!summarize_method %in% c('none', 'mean', 'median', 'min', 'max'))
            {
                stop('summarize_method must be one of `c("none", "mean", ',
                     '"median", "min", "max")`')
            }
        }
    }
    ...
    # add points
    plot_ <- plot_ + stat_summary(fun.y = summarize_method, geom = 'point')

...
proc sort data=safty.adsl_ex(where=(trt01a ^= "")) nodupkey out=adsl;
   by usubjid;
run;
proc sort data=safty.adae_ex out=adae;
   by usubjid;
run;
...
PROC EXPORT DATA= WORK.all 
            OUTFILE= "P:\Safety Signal Detection\Stat.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
PROC IMPORT OUT= WORK.all 
            DATAFILE= "P:\Safety Signal Detection\Stat.csv" 
            DBMS=CSV REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
RUN;
...
PROC IMPORT OUT= WORK.result 
            DATAFILE= "P:\Safety Signal Detection\Output_20171110.csv" 
            DBMS=CSV REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
RUN;
...
proc compare CRITERION=0.00015 method=absolute
             data=all(drop = Table Name1 Label1 cValue1 HazardRatio HR_Lower HR_Upper)

...
The `haven` package can read in data stored in SAS, SPSS and Stata format. You need to load the `haven` package with ?library before using it.

1. Load the `haven` package, and then read in the file `adae.sas7bdat`. 
2. View the resulting tibble both on the command line, and with the ?View command. Are there any differences?
...
1. Read in the file `adae.sas7bdat`.

Use ggplot to create two charts of the data:
 * A histogram of patient ages
 * A bar chart of patient races.
 
Use dplyr to answer two statistical questions about the data:
 * How many males and females were in the study? 
 * What is the average age of females in each cohort?
